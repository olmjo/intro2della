...
#+DESCRIPTION:
#+STYLE: <style>html {font-family: sans-serif;}</style>
#+STYLE: <style>code {font-family: monospace; border: 1pt solid #AEBDCC; background-color: #F3F5F7; padding: 1pt; font-size: 90%;}</style>
#+STYLE: <style>pre {font-family: monospace;}</style>
#+OPTIONS: H:4 num:t toc:nil @:t ::t |:t ^:t f:t TeX:t
...
* Introduction
*[[http://www.princeton.edu/researchcomputing/computational-hardware/machine-2/][
{{{machine(Della)}}}
]]*
is one of several clusters managed by the
[[http://www.princeton.edu/researchcomputing/index.xml][Research Computing]]
collaboration at Princeton University. Currently,
#
{{{machine(Della)}}}
#
...
** Prerequisites
   ...
   #
   . Also,
   it is assumed that you have the means to use the
   #
   {{{software(ssh)}}}
   #
   protocol
...
** Using Disk Space on Della (and other Princeton HPCRC Systems)
   ...
   | =/scratch/pvfs2/=   | Yes          | Yes         | No        | within-job fast I/O, old files rm-ed |
   | =/tigress-hsm/=     | Yes          | Yes         | Yes       | post-analysis, persistent storage |
...
First, for the project in general,
 ...
 3) mirror the input data from =/tigress-hsm/= onto =/scratch/network/= (or =/scratch/pvfs2/=).

#+LaTeX:\noindent
Next, for any given job
 1) update the mirror on =/scratch/network/= (or =/scratch/pvfs2/) with =rsync=.
 2) =rsync= the input data from =/scratch/network/= to =/scratch/pvfs2/= if you
 ...
 3) run your program
    - read as necessary from =/scratch/network/=, =/scratch/pvfs2/=
    - save any within-process data to =/scratch/=
    - save any within-job, across-process data to =/scratch/pvfs2/= or
      =/scratch/network/= depending on need
 4) clean up any within-process data on =/scratch/=
 5) =rsync= the output data from =/scratch/pvfs2/= to =/scratch/network=
...
*** Example
    ...
    2) The PBS script then checks that the =/scratch/network/= (or
       =/scratch/pvfs2=) mirror of the =/tigress-hsm/= copy of the data is in
       sync.
...
consider saving this output in =/scratch/pvfs2/=.
